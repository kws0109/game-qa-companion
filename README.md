...
그래서 게임 자동화는 좌표 하드코딩과 수작업 템플릿 캡처에 묶인다. 이 도구는 화면을 분석해
**UI 계층을 vision + LLM으로 합성**한다. 엔진에 플러그인을 심는 침투형 접근을 쓸 수 없는
환경을 위한 비침투(non-invasive) 방식이다.
...
검출과 라벨링까지는 AI가 처리하지만, 어떤 요소가 정답인지는 QA가 확정한다. 확정된 요소는
이후 모든 자동 인식의 기준이 되고, LLM의 재해석은 코드에서 차단된다.
...
- **확정 요소는 LLM이 못 덮어쓴다** — 프롬프트 지시가 아니라 코드가 LLM 응답 적용을 차단한다.
...
나이트 크로우 PC 클라이언트로 실행한 결과. 상세 캡션은 [demo/](demo/) 참조.

**LLM 정리 — 비UI 164개 제거 (161개 → 8개)**

| 정리 전 (161개) | 정리 후 (8개) |
|---|---|
| ![before](demo/01_llm_cleanup_before.png) | ![after](demo/02_llm_cleanup_after.png) |

캐릭터 선택 화면. CV 검출이 캐릭터 모델·배경 텍스처까지 161개를 잡으면, LLM 라벨링이 비UI를 `unknown`으로 판정해 제거하고 실제 UI 8개(버튼·제목·패널)만 남긴다.

**복잡한 인게임 화면 — UI 82개 식별**

![field](demo/03_complex_field_detection.png)

인게임 필드. 미니맵·상단 바·버프·스킬 슬롯 등을 잡는다. 들판 텍스처에 작은 오탐이 일부 남아, 검출 단계에서 오탐률을 줄이는 방법을 계속 실험 중이다. 현재는 vision+LLM 조합의 방향성을 확인하는 프로토타입 단계다.
...
AI 코딩 에이전트(Claude Code)와 협업해 개발했다. 요구사항 정의와 아키텍처 선택, 임계값
실측 튜닝, 실기기 검증은 직접 했다.
